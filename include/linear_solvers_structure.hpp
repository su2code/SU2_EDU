...
public:
   ...
   * \param[in] monitoring - turn on priting residuals from solver to screen.
   */
  void MultiGrid_LinSolver(CSysMatrix **Jacobian, CSysVector **LinSysRes, CSysVector **LinSysSol, CMatrixVectorProduct & mat_vec, CGeometry **geometry, CConfig *config, unsigned short iMesh, unsigned short mu, double tol, unsigned long m, bool monitoring);
  
  /*!
   * \brief Restricted Residual from fine grid to a coarse grid
   */
  void SetRestricted_Residual(CSysVector *res_fine, CSysVector *res_coarse, CGeometry *geo_coarse, CConfig *config);

  /*!
   * \brief Prolongated Solution from the coarse grid to a fine grid
   */
   ...
   * \param[in] monitoring - turn on priting residuals from solver to screen.
   */
  unsigned long Solve(CSysMatrix **Jacobian, CSysVector **LinSysRes, CSysVector **LinSysSol, CGeometry **geometry,
